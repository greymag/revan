...
class SloleksXmlConverter(private val config: Config = Config.DEFAULT) {
    companion object {
    ...
    fun loadAndConvert(file: File): List<SloleksWord> {
        return parseXmlToWords(file.inputStream())
    }

    private fun parseXmlToWords(input: InputStream): List<SloleksWord> {
        val result = mutableListOf<SloleksWord>()

        val factory = SAXParserFactory.newInstance()
        val saxParser = factory.newSAXParser()

        saxParser.parse(input, HandlerImpl(config) { result.add(it) })
